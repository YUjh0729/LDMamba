...
from .D3D.dcn.modules.deform_conv import DeformConvPack


class OneWay_MambaLayer(nn.Module):
    def __init__(self, dim, p, d_state=16, d_conv=4, expand=2):
        ...
        # 设置 Mamba 层的 out_proj 为 Identity，不做变换
...
class CBAM3D(nn.Module):
    def __init__(self, in_channal, ratio=16, kernel_size=7):
        super(CBAM3D, self).__init__()
        self.ca = ChannelAttention3D(in_channal, ratio)  # 3D通道注意力实例
        self.sa = SpatialAttention3D(kernel_size)  # 3D空间注意力实例
...
class LDMambaNet_3d(nn.Module):
    def __init__(self,
        ...
        # 获取输入的H,W
        self.deformable = DeformConvPack(in_channels=features_per_stage[-1], out_channels=features_per_stage[-1],
                                         kernel_size=5, stride=1, padding=2, groups=features_per_stage[-1])
...
def get_LDMambaNet_from_plans_3d(plans_manager: PlansManager,
                           dataset_json: dict,
    ...
    if network_class == LDMambaNet_3d:
